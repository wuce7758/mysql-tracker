...
public class HandlerKafkaZkLocalPerformance implements MagpieExecutor {
    ...
    public void run() throws Exception {
        ...
        //!!!!!!!!!!!!!!!!!!!!!!!!!!debug
        switchEnd = System.currentTimeMillis();
        if(switchStart > 0) {
            trackerSwitchTime += (switchEnd - switchStart);
        }
        whileStart = System.currentTimeMillis();
        ...
        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!debug
        whileEnd = System.currentTimeMillis();
        trackerWhileTime += (whileEnd - whileStart);
        persisStart = System.currentTimeMillis();
        //per minute record
        if(lastEntry != null) {
        ...
        //!!!!!!!!!!!!!!!!!!!!!!!!debug
        persisEnd = System.currentTimeMillis();
        trackerPersisTime += (persisEnd - persisStart);
        ...
        if(monitor.persisNum > 0) {
            ...
            //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!debug
            logger.info("---> !!!!debug , while time : " + trackerWhileTime);
            trackerWhileTime = 0;
            logger.info("---> !!!!debug , persis time : " + trackerPersisTime);
            trackerPersisTime = 0;
            logger.info("---> !!!!debug , monitor time : " + trackerMonitorTime);
            trackerMonitorTime = 0;
            logger.info("---> !!!!debug , switch time : " + trackerSwitchTime);
            trackerSwitchTime = 0;
            logger.info("---> blocking queue size:" + entryQueue.size());
        ...
        trackerMonitorTime += (monitorEnd - monitorStart);
