...
/**
 * Binary log dbsync.event definitions. This includes generic code common to all types
 * of log events, as well as specific code for each type of log dbsync.event.
 * 
...
public abstract class LogEvent
{
     ...
     * start, other) to be sure an dbsync.event is not executed more than once in a
     ...
     * dbsync.event should be skipped or not. Example of ID: we already have the server id
     * (4 bytes), plus:
     * timestamp_when_the_master_started (4 bytes), a counter (a sequence number
     * which increments every time we write an dbsync.event to the binlog) (3 bytes).
    ...
    /* dbsync.event-specific post-header sizes */
    ...
    /**
     * These dbsync.event numbers were used for 5.1.0 to 5.1.15 and are therefore
     * obsolete.
     */
    ...
    /**
     * These dbsync.event numbers are used from 5.1.16 and forward
     */
    ...
    /**
     * Heartbeat dbsync.event to be send by master at its idle time to ensure master's online status to slave
     */
    ...
    /* New Maria dbsync.event numbers start from here */
    public static final int    ANNOTATE_ROWS_EVENT      = 160;
    /*
    Binlog checkpoint dbsync.event. Used for XA crash recovery on the master, not used
    in replication.
    A binlog checkpoint dbsync.event specifies a binlog file such that XA crash
    ...
    /*
    Gtid dbsync.event. For global transaction ID, used to start a new dbsync.event group,
    instead of the old BEGIN query dbsync.event, and also to mark stand-alone
    events.
     */
    public static final int    GTID_EVENT               = 162;
    /*
    Gtid list dbsync.event. Logged at the start of every binlog, to record the
    ...
    */
    ...
    /**
        For an dbsync.event, 'e', carrying a type code, that a slave,
        ...
        found an unknown dbsync.event in the relay log.
     */
