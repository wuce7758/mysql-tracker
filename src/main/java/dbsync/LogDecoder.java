 ...
 * 
 * <pre>
 * dbsync.LogDecoder decoder = new dbsync.LogDecoder();
 * decoder.handle(...);
 * 
 * dbsync.LogEvent dbsync.event;
 * do
 * {
 *     dbsync.event = decoder.decode(buffer, context);
 * 
 *     // process log dbsync.event.
 * }
 * while (dbsync.event != null);
...
public final class LogDecoder
{
    ...
    /**
     * Decoding an dbsync.event from binary-log buffer.
     * 
     * @return <code>UknownLogEvent</code> if dbsync.event type is unknown or skipped,
     *         <code>null</code> if buffer is not including a full dbsync.event.
     */
        ...
        if (limit >= FormatDescriptionLogEvent.LOG_EVENT_HEADER_LEN)
        {
            ...
            if (limit >= len)
            {
                ...
                if (handleSet.get(header.getType()))
                {
                    ...
                    try
                    {
                        /* Decoding binary-log to dbsync.event */
    ...
    /**
     * Deserialize an dbsync.event from buffer.
     * 
     * @return <code>UknownLogEvent</code> if dbsync.event type is unknown or skipped.
     */
            ...
            LogContext context) throws IOException
    {
        ...
        case LogEvent.SLAVE_EVENT: /* can never happen (unused dbsync.event) */
            {
                if (logger.isWarnEnabled())
        ...
        default:
            ...
            if((buffer.getUint16(LogEvent.FLAGS_OFFSET) & LogEvent.LOG_EVENT_IGNORABLE_F) > 0){
                ...
                if (logger.isWarnEnabled())
                    logger.warn("Skipping unrecognized binlog dbsync.event "
        ...
        /* updating position in context */
        logPosition.position = header.getLogPos();
        /* Unknown or unsupported log dbsync.event */
