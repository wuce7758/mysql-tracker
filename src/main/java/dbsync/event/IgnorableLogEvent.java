 ...
 *   Base class for ignorable log events. Events deriving from
  ...
  the old slaves that support ignorable log dbsync.event mechanism
  (they will just ignore unrecognized ignorable events).

  @note The only thing that makes an dbsync.event ignorable is that it has
  the LOG_EVENT_IGNORABLE_F flag set.  It is not strictly necessary
  that ignorable dbsync.event types derive from Ignorable_log_event; they may
