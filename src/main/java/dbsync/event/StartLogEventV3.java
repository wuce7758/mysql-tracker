 ...
 * Start_log_event of binlog format 4 (MySQL 5.0), that is, the dbsync.event that
 * describes the other events' Common-Header/Post-Header lengths. This dbsync.event is
 * sent by MySQL 5.0 whenever it starts sending a new binlog if the requested
 * position is >4 (otherwise if ==4 the dbsync.event will be sent naturally).
 * 
...
public class StartLogEventV3 extends LogEvent
{
    ...
    /* start dbsync.event post-header (for v3 and v4) */
