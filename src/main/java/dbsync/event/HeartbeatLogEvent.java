...
/**
 * <pre>
 * Replication dbsync.event to ensure to slave that master is alive.
 *   The dbsync.event is originated by master's dump thread and sent straight to
 *   slave without being logged. Slave itself does not store it in relay log
 *   but rather uses a data for immediate checks and throws away the dbsync.event.
 * 
 *   Two members of the class log_ident and Log_event::log_pos comprise 
 *   @see the event_coordinates instance. The coordinates that a heartbeat
 *   instance carries correspond to the last dbsync.event master has sent from
 *   its binlog.
 * </pre>
 * 
